...
std::shared_ptr<ServiceRegistry> make_registry() {
    ...
    handlers["EchoError"] = [](const std::string&)
        -> std::pair<std::string, std::error_code> {
        return {"", std::make_error_code(std::errc::io_error)};
    };

    handlers["EchoThrow"] = [](const std::string&)
        -> std::pair<std::string, std::error_code> {
        throw std::runtime_error("handler exploded");
        return {"", {}};
    };

    registry->register_service(std::make_shared<FakeService>("EchoService", handlers));
    return registry;
}

/// Helper: build a minimal REQUEST envelope.
ibridger::Envelope make_request(uint64_t id,
...
TEST(Dispatcher, SuccessfulDispatchReturnsHandlerOutput) {
    ...
    auto response = dispatcher.dispatch(
        make_request(1, "EchoService", "Echo", "hello"));
...
TEST(Dispatcher, UnknownServiceReturnsNotFound) {
    ...
    auto response = dispatcher.dispatch(
        make_request(42, "ghost.Service", "DoThing"));
...
TEST(Dispatcher, UnknownMethodReturnsNotFound) {
    ...
    auto response = dispatcher.dispatch(
        make_request(7, "EchoService", "NonExistentMethod"));

    EXPECT_EQ(response.type(),       ibridger::ERROR);
    EXPECT_EQ(response.status(),     ibridger::NOT_FOUND);
    EXPECT_EQ(response.request_id(), 7ULL);
    EXPECT_NE(response.error_message().find("NonExistentMethod"), std::string::npos);
...
TEST(Dispatcher, HandlerErrorReturnsInternal) {
    ...
    auto response = dispatcher.dispatch(
        make_request(99, "EchoService", "EchoError"));
...
TEST(Dispatcher, HandlerExceptionReturnsInternalNeverCrashes) {
    ...
    auto response = dispatcher.dispatch(
        make_request(55, "EchoService", "EchoThrow"));

    EXPECT_EQ(response.type(),       ibridger::ERROR);
    EXPECT_EQ(response.status(),     ibridger::INTERNAL);
    EXPECT_EQ(response.request_id(), 55ULL);
    EXPECT_NE(response.error_message().find("handler exploded"), std::string::npos);
}
...
TEST(Dispatcher, RequestIdAlwaysPreserved) {
    ...
    EXPECT_EQ(dispatcher.dispatch(
        make_request(kId, "EchoService", "Echo", "x")).request_id(), kId);

    // Unknown service
    EXPECT_EQ(dispatcher.dispatch(
        make_request(kId, "no.Service", "M")).request_id(), kId);

    // Unknown method
    EXPECT_EQ(dispatcher.dispatch(
        make_request(kId, "EchoService", "NoMethod")).request_id(), kId);

    // Handler error
    EXPECT_EQ(dispatcher.dispatch(
        make_request(kId, "EchoService", "EchoError")).request_id(), kId);

    // Handler throws
    EXPECT_EQ(dispatcher.dispatch(
        make_request(kId, "EchoService", "EchoThrow")).request_id(), kId);
}
